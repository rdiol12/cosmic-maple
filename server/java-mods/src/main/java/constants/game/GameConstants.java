...
public class GameConstants {
    ...
    public static int getHallOfFameMapid(Job job) {
        ...
        if (isCygnus(jobid)) {
            ...
            } else if (job.isA(Job.SAGE)) {
                return MapId.HALL_OF_MAGICIANS;  // Sages share with Magicians for now
            } else if (job.isA(Job.NECROMANCER)) {
                return MapId.HALL_OF_MAGICIANS;  // Necromancers share with Magicians for now
            } else {
    ...
    public static boolean isSage(final int job) {
        return job >= 600 && job <= 612;
    }

    public static boolean isNecromancer(final int job) {
        return job >= 700 && job <= 712;
    }
