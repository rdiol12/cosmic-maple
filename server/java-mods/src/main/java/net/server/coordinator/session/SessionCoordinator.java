...
public class SessionCoordinator {
    ...
    public AntiMulticlientResult attemptGameSession(Client client, int accountId, Hwid hwid) {
        ...
        log.debug("[BOT-DEBUG] attemptGameSession: remoteHost='" + remoteHost + "' remoteAddr='" + client.getRemoteAddress() + "' hwid=" + hwid + " accId=" + accountId);
        if (!YamlConfig.config.server.DETERRED_MULTICLIENT) {
            ...
            log.debug("[BOT-DEBUG] attemptGameSession: added cache entries for remoteHost='" + remoteHost + "' and remoteAddr='" + client.getRemoteAddress() + "'");
            return AntiMulticlientResult.SUCCESS;
        }
    ...
    public Hwid pickLoginSessionHwid(Client client) {
        ...
        log.debug("[BOT-DEBUG] pickLoginSessionHwid: looking up remoteHost='" + remoteHost + "' cacheKeys=" + hostHwidCache.debugGetKeys());
        // thanks BHB, resinate for noticing players from same network not being able to login
        Hwid result = hostHwidCache.removeEntryAndGetItsHwid(remoteHost);
        log.debug("[BOT-DEBUG] pickLoginSessionHwid: result=" + result);
        return result;
    }
