...
public final class PlayerLoggedinHandler extends AbstractPacketHandler {
    ...
    public final void handlePacket(InPacket p, Client c) {
        ...
        log.debug("[BOT-DEBUG] PlayerLoggedinHandler: cid=" + cid + " remoteAddr=" + c.getRemoteAddress() + " world=" + c.getWorld() + " channel=" + c.getChannel());

        if (!c.tryacquireClient()) {
            // thanks MedicOP for assisting on concurrency protection here
            log.debug("[BOT-DEBUG] FAIL: tryacquireClient failed for cid=" + cid);
        ...
        try {
            ...
            if (wserv == null) {
                log.debug("[BOT-DEBUG] FAIL: world is null for cid=" + cid + " world=" + c.getWorld());
            ...
            if (cserv == null) {
                ...
                if (cserv == null) {
                    log.debug("[BOT-DEBUG] FAIL: channel is null for cid=" + cid);
                    c.disconnect(true, false);
                    return;
                }
            }
            ...
            log.debug("[BOT-DEBUG] playerStorageLookup: cid=" + cid + " found=" + (player != null));

            final Hwid hwid;
            if (player == null) {
                hwid = SessionCoordinator.getInstance().pickLoginSessionHwid(c);
                log.debug("[BOT-DEBUG] pickLoginSessionHwid: remoteAddr=" + c.getRemoteAddress() + " hwid=" + hwid);
                if (hwid == null) {
                    log.debug("[BOT-DEBUG] FAIL: hwid is null (pickLoginSessionHwid returned null) for cid=" + cid);
                    c.disconnect(true, false);
                    return;
                }
            } else {
            ...
            if (!server.validateCharacteridInTransition(c, cid)) {
                log.debug("[BOT-DEBUG] FAIL: validateCharacteridInTransition failed for cid=" + cid);
                c.disconnect(true, false);
                return;
            }
            log.debug("[BOT-DEBUG] validateCharacteridInTransition passed for cid=" + cid);
            ...
            if (player == null) {
                try {
                    ...
                    newcomer = true;
                    log.debug("[BOT-DEBUG] loadCharFromDB: cid=" + cid + " loaded=" + (player != null) + (player != null ? " name=" + player.getName() : ""));
                } catch (SQLException e) {
                    log.debug("[BOT-DEBUG] FAIL: loadCharFromDB threw SQLException for cid=" + cid);
                    e.printStackTrace();
                } catch (Exception e) {
                    log.debug("[BOT-DEBUG] FAIL: loadCharFromDB threw Exception for cid=" + cid + " : " + e.getClass().getName() + " : " + e.getMessage());
                    e.printStackTrace();
                }

                if (player == null) { //If you are still getting null here then please just uninstall the game >.>, we dont need you fucking with the logs
                    log.debug("[BOT-DEBUG] FAIL: player is null after loadCharFromDB for cid=" + cid);
                    c.disconnect(true, false);
                    return;
                }
            }
            ...
            if (tryAcquireAccount(accId)) { // Sync this to prevent wrong login state for double loggedin handling
                try {
                    ...
                    log.debug("[BOT-DEBUG] loginState check: cid=" + cid + " state=" + state + " LOGIN_SERVER_TRANSITION=" + Client.LOGIN_SERVER_TRANSITION + " allowLogin=" + allowLogin);
                    if (state != Client.LOGIN_SERVER_TRANSITION || !allowLogin) {
                        log.debug("[BOT-DEBUG] FAIL: login state mismatch for cid=" + cid + " state=" + state);
                        c.setPlayer(null);
                        c.setAccID(0);
                    ...
                    log.debug("[BOT-DEBUG] SUCCESS: updating login state to LOGGEDIN for cid=" + cid);
            ...
            log.debug("[BOT-DEBUG] SUCCESS: player " + player.getName() + " (cid=" + cid + ") added to channel " + c.getChannel() + " and world " + c.getWorld() + " mapId=" + player.getMapId());
