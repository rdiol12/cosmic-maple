...
import server.life.BotBehavior;
    ...
    public void handlePacket(InPacket p, Client c) {
        ...
        if (CommandsExecutor.isCommand(c, s)) {
            ...
            if (!chr.isHidden()) {
                ...
                // Notify at most 1 random bot on the same map to respond (avoids spam)
                BotManager bm = BotManager.getInstance();
                java.util.List<BotBehavior> mapBots = new java.util.ArrayList<>();
                for (Character mapChr : chr.getMap().getCharacters()) {
                    if (bm.isBot(mapChr.getId())) {
                        BotBehavior beh = bm.getBehavior(mapChr.getId());
                        if (beh != null) mapBots.add(beh);
                    }
                }
                if (!mapBots.isEmpty()) {
                    try {
                        mapBots.get(new java.util.Random().nextInt(mapBots.size()))
                            .onPlayerChat(chr.getName(), s);
                    } catch (Exception e) { /* bot may have been removed */ }
                }
            } else {
