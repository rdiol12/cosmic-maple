...
public final class WhisperHandler extends AbstractPacketHandler {
    ...
    private void handleWhisper(String message, Character user, Character target) {
        ...
        // Bot whisper handling — respond via map chat since bots can't receive packets
        BotManager bm = BotManager.getInstance();
        if (bm.isBot(target.getId())) {
            BotBehavior beh = bm.getBehavior(target.getId());
            if (beh != null) {
                beh.onWhisper(user.getName(), message);
            }
