    ...
    public final void handlePacket(InPacket p, Client c) {
        ...
        switch (operation) {
            ...
            case 4: { // invite
                ...
                if (server.life.BotManager.getInstance().isBotName(name)) {
                    // Route party invite to bot AI
                    server.life.BotManager bm = server.life.BotManager.getInstance();
                    Character botChr = world.getPlayerStorage().getCharacterByName(name);
                    if (botChr != null && party != null) {
                        server.life.BotBehavior behavior = bm.getBehavior(botChr.getId());
                        if (behavior != null) {
                            behavior.onPartyInvite(player, party.getId());
                        }
                    } else if (botChr != null && party == null) {
                        // Create party first, then invite bot
